...
public:
    ...
    table()
    {
        hash = {"keywords", "identifiers", "delimiters", 
        "constants", "operators"};

        tokens.resize(hash.size());
    }

    bool addToken(std::string token_)
    {
        // keywords
        if(is_keyword(token_))
        {
            tokens[0].push_back(token_);
            return true;
        }

        // operators
        if(is_operator(token_))
        {
            tokens[4].push_back(token_);
            return true;
        }

        // delimiters
        if(is_delimiter(token_))
        {
            tokens[2].push_back(token_);
            return true;
        }

        // constants
        if(is_constant(token_))
        {
            tokens[3].push_back(token_);
            return true;
        }

        // identifiers
        if(is_identifier(token_))
        {
            tokens[1].push_back(token_);
            return true;
        }
    ...
    void print_table()
    {
        ...
        std::cout << "------------------------------------------------------------------------------\n";
        std::cout << hash[0] << "    | ";
        print_1d_string_vec(tokens[0]);
        std::cout << hash[1] << " | ";
        print_1d_string_vec(tokens[1]);
        std::cout << hash[2] << "  | ";
        print_1d_string_vec(tokens[2]);
        std::cout << hash[3] << "   | ";
        print_1d_string_vec(tokens[3]);
        std::cout << hash[4] << "   | ";
        print_1d_string_vec(tokens[4]);
        std::cout << "------------------------------------------------------------------------------\n";
    }
...
int main()
{
    ...
    token_list = reduce_vec(token_list);
