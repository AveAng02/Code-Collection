...
string IP(string ct64)
{
    ...
    {
        for(int j=0;j<8;j++)
            perm_ct64+=ct64[ip[i][j]-1];
    }

    return perm_ct64;
}

string EP(string rct)
{
...
int main()
{
    ...
    string ct64=convertHextoBin(cipher_text);
    string perm_ct64=IP(ct64);

    string res_str=perm_ct64;
    for(int i=0;i<16;i++)
    {
        string lct=res_str.substr(0,32);
        string rct=res_str.substr(32,32);

        string rct48=EP(rct);
        string xor48=XOR(rct48,key48Array[15-i]);
        string sbox_Output=sBoxComputation(xor48);
        string perm_sbox=P32(sbox_Output);
        string right_res=XOR(perm_sbox,lct);

        res_str=rct+right_res;
    }
    string rev64=res_str.substr(32,32)+res_str.substr(0,32);
    string plain_text=convertBintoHex(FP(rev64));
